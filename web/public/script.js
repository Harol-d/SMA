...
async function analyzeDelays() {
    ...
    try {
        ...
        displayAnalysisResults('Análisis de Atrasos', data.analysis || data);
    } catch (error) {
...
async function analyzePendingTasks() {
    ...
    try {
        ...
        displayAnalysisResults(title, data.tasks || data);
    } catch (error) {
        console.error('Error:', error);
...
async function getProjectSummary() {
    ...
    try {
        ...
        displayAnalysisResults(`Resumen del Proyecto: ${projectName}`, data.summary || data);
...
function handleFileDrop(event) {
    ...
    if (files.length > 0) {
        ...
        if (file.type.includes('sheet') || file.name.endsWith('.xlsx') || file.name.endsWith('.xls') ||
            ...
            showErrorMessage('Por favor selecciona un archivo válido (.xlsx, .xls, .pdf, .docx, .doc, .txt)');
        }
    }
}

// Procesar archivo (Excel, PDF, Word, texto)
async function processExcelFile(file) {
    ...
    try {
        ...
        hideUploadProgress();
        updateFileStatus();
        showSuccessMessage('Archivo cargado exitosamente');
