...
// Configurar multer para archivos Excel, PDF, Word y texto
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    // Aceptar archivos Excel, PDF, Word y texto
    const allowedTypes = [
    ...
    if (allowedTypes.includes(file.mimetype) || 
      ...
      cb(new Error('Solo se permiten archivos Excel (.xlsx, .xls), PDF (.pdf), Word (.docx, .doc) y texto (.txt)'));
    }
  }
});
...
app.post('/api/upload_excel', upload.single('file'), async (req, res) => {
  try {
    ...
    const response = await axios.post(`${backendURL}/api/cargar`, formData, {
      headers: {
        ...formData.getHeaders(),
